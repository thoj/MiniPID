...
double MiniPID::getOutput(double actual, double setpoint)
{
    ...
    if (hysteresisControl) {
        if (!reversed && setpoint - actual > hysteresisOn)
            return maxOutput;
        if (!reversed && actual - setpoint > hysteresisOff)
            return minOutput;
        if (reversed && setpoint - actual > hysteresisOn)
            return minOutput;
        if (reversed && actual - setpoint > hysteresisOff)
    ...
    if (minOutput != maxOutput && !bounded(Poutput + Foutput + iSum, minOutput, maxOutput)) {
        if (Poutput + Foutput + iSum > maxOutput)
            iSum = maxOutput - (Poutput + Foutput);
        else
            iSum = minOutput - (Poutput + Foutput);
    }
    else if (outputRampRate != 0 && !bounded(output, lastOutput - outputRampRate, lastOutput + outputRampRate)) {
        if (isPositive(error) != isPositive(iSum))
            iSum += I * error;
    }
    else {
        iSum += I * error;
